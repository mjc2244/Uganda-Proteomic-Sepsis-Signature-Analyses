...
results_ebb <- topTable(fit2, coef="C2vsC1", adjust.method="fdr", sort.by="P", number=Inf)

#Volcano plot
library(EnhancedVolcano)
# Create a data frame for the volcano plot
results_ebb$Prot <- toupper(rownames(results_ebb))  # Create a column for gene names if not already present
names(results_ebb$logFC) <- toupper(results_ebb$Prot)
names(results_ebb$P.Value) <- toupper(results_ebb$Prot)

results_ebb$selectedLab <- ifelse(results_ebb$adj.P.Val < 0.05 & abs(results_ebb$logFC) > 1, as.character(results_ebb$Prot), NA)
EnhancedVolcano(
  results_ebb,
  lab = NA,
  x = 'logFC',
  ...
  data = subset(results_ebb, !is.na(selectedLab)),  # Only use rows where labels are not NA
